...
def _legend_handles():
    ...
    return [
        ...
        Patch(
            facecolor=colors["robot (empty)"], edgecolor="none", label="Robot (empty)"
        ),
        Patch(
            facecolor=colors["robot (loaded)"], edgecolor="none", label="Robot (loaded)"
        ),
    ]
...
def save_frames_to_svg(
    ...
    for i, frame in enumerate(frames):
        ...
        fig.savefig(
            Path(output_dir) / f"frame_{i:04d}.svg", dpi=dpi, bbox_inches="tight"
        )
        plt.close(fig)
    fig, ax = plt.subplots()
    ax.set_axis_off()
...
def replay_animation(
    ...
    def update(i: int):
        ...
        return (
            [im, step_text]
            + list(lines.values())
            + ([battery_bar] if battery_bar else [])
        )
